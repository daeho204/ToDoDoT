...
<body>
	...
	<div id="wrapper">
		...
		<ul class="actions vertical">
			...
			<form action="JoinController" method="post">
				<li><input type="text" id="input" name="id" placeholder="ID를 입력하세요"></li>
	...
	<script>
		function idCheck(){
			...
			$.ajax({
				type : "get", // 요청방식
				data : {"id" : input.value}, // 요청데이터 = JSON타입 : { "key" : "value"}
				url : "IdCheckController", // 요청경로
				dataType : "text", // 응답받을 데이터 형식
				success : function (data) { // parameter data 는 서버에서 응답한 데이터
					console.log(data)
					if(data=="YY"){ //넘어오는 데이터가 "YY" 일떄 : 사용가능한 아이디
						$("#sp").text("사용 가능한 아이디 입니다.") //id 가 "sp"인 span 에 넣고싶은 텍스트값
					}else{ // 넘어오는 데이터가 "NN"일때 : 사용불가능한 아이디
						$("#sp").text("사용할 수 없는 아이디 입니다") 
					}
